...
public class BoardDAO {
	...
	public List<BoardDTO> select(int board_num, int page_num){
		...
		String query =  "SELECT * FROM ( SELECT ROWNUM RNUM, TB.* FROM ( SELECT * FROM POSTDB P JOIN BOARDDB B ON (P.BOARD_NUM = B.BOARD_NUM) WHERE P.BOARD_NUM = '" + board_num + "' ORDER BY POST_NUM DESC )TB )WHERE RNUM BETWEEN '" + ((page_num - 1) * 10 + 1) + "' AND '" + (page_num * 10) + "'";
